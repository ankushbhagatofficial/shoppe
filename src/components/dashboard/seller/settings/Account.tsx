...
export default function Account({ seller }: { seller: any }) {
  ...
  const updateEmail = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.patch("/api/seller/settings/email", data)
      if (res.status === 200) {
  ...
  const updatePassword = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await axios.patch("/api/seller/settings/password", data)
      if (res.status === 200) {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const res = await axios.delete("/api/seller")
